    ...
    openProductModal(productId) {
        ...
        modalBody.innerHTML = `
            ...
            <div class="showcase-img-top-container">
                <img src="${product.image}" alt="${product.name}" class="showcase-top-img">
            </div>
            ...
            <div style="display:flex; flex-direction:column; gap:15px; margin-bottom:20px; background:var(--light-grey); padding:20px; border:1px solid var(--border-grey);">
                <div>
                    <label style="font-size:0.85rem; font-weight:700; text-transform:uppercase; display:block; margin-bottom:8px;">Select Size:</label>
                    <div style="display:flex; gap:8px; flex-wrap:wrap;">
                        ${product.sizes.map((s, idx) => `
                            <button class="modal-opt-btn size-opt-btn ${idx === 0 ? 'active' : ''}" onclick="app.setModalSize(this, '${s}')">${s}</button>
                        `).join('')}
                    </div>
                </div>
                <div>
                    <label style="font-size:0.85rem; font-weight:700; text-transform:uppercase; display:block; margin-bottom:8px;">Available Color:</label>
                    <div style="display:flex; gap:8px; flex-wrap:wrap;">
                        ${product.colors.map((c, idx) => `
                            <button class="modal-opt-btn color-opt-btn ${idx === 0 ? 'active' : ''}" onclick="app.setModalColor(this, '${c}')">${c}</button>
                        `).join('')}
                    </div>
                </div>
            </div>
            ...
            <div style="display:flex; gap:15px; margin-top:25px; flex-wrap:wrap;">
                <button class="btn-primary" style="flex:1;" onclick="app.addFromModal('${product.id}')">Add To Shopping Bag</button>
                <button class="btn-primary btn-gold" style="flex:1;" onclick="app.buyNowFromModal('${product.id}')">Buy Now</button>
            </div>
        `;
